...
import { SERVICE_ROUTES, getAllServiceNames, getAllServicePrefixes } from '../constants/services';
import type { HealthCheckResponse, NotFoundResponse, ServiceName } from '../types/api';
...
const createServiceRouteHandler = (serviceName: ServiceName) => {
  return async (c: Context) => {
    ...
    return ProxyService.proxyRequest(c, serviceName, targetPath);
  };
};

// Register all service routes dynamically
SERVICE_ROUTES.forEach(({ prefix, service }) => {
  gateway.all(`${prefix}/:domain/:function{.*}`, createServiceRouteHandler(service));
});
